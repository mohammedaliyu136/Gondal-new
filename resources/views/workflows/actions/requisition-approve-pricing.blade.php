  ...
  <div class="card-head" style="background: #eff6ff; padding: 10px 16px;">
    <div>
      ...
      <h4 style="margin:0; font-size:0.95rem; color: #1e40af;">
        <span style="margin-right:6px;">&#128176;</span> Stage Action: {{ $actionHandler?->label() ?? 'Approve Final Pricing & Quantities' }}
      </h4>
      <p style="margin:0; font-size:0.8rem; color:#475569;">
        {{ $actionHandler?->description() ?? 'Review line items, confirm or reduce approved quantities, and set the final approved unit prices before approving.' }}
      </p>
    </div>
  </div>
  <div class="card-body flush">
    <div class="table-wrap">
      <table class="table" id="req-price-table-{{ $instance->id }}" style="margin-bottom:0; font-size:0.875rem;">
        <thead>
          <tr style="background:#f1f5f9;">
            <th style="width:15%;">Decision <span class="req">*</span></th>
            <th style="width:30%;">Requested Item &amp; Purpose</th>
            <th style="width:15%;" class="num">Approved Qty <span class="req">*</span></th>
            <th style="width:10%;">Unit</th>
            <th style="width:16%;" class="num">Approved Price (₦) <span class="req">*</span></th>
            <th style="width:14%;" class="num">Approved Total (₦)</th>
          </tr>
        </thead>
        <tbody id="req-price-tbody-{{ $instance->id }}">
            ...
            <tr class="req-item-row" data-row-id="{{ $idx }}">
              <td style="vertical-align:top;">
                <select name="stage_action_items[{{ $idx }}][status]"
                        ...
                        onchange="toggleRequisitionPricingRowStatus(this, '{{ $instance->id }}')"
              ...
              <td style="vertical-align:top;">
                ...
                @endif
                <div style="font-size:0.72rem; color:var(--muted, #64748b); margin-top:2px;">
                  Requested: {{ rtrim(rtrim((string) $item->quantity, '0'), '.') }} {{ $item->unit }} @ {{ \App\Support\Money::format($item->unit_price_minor) }}
                </div>
              ...
              <td style="vertical-align:top;">
                <input type="number" step="any" min="0.01" name="stage_action_items[{{ $idx }}][quantity]"
                       ...
                       oninput="recalcRequisitionPricingTotal('{{ $instance->id }}')"
        ...
        <tfoot>
          <tr style="background:#f8fafc; font-weight:bold;">
            <td colspan="5" style="text-align:right; padding-right:12px;">Total Approved Requisition Amount:</td>
            <td class="num font-bold" id="req-price-total-{{ $instance->id }}" style="text-align:right; color:#1e40af; font-size:0.95rem;">
              {{ \App\Support\Money::format($requisition->approved_total_minor ?? $requisition->total_minor) }}
            </td>
          </tr>
        </tfoot>
      </table>
...
<script>
  window.toggleRequisitionPricingRowStatus = function(selectEl, instanceId) {
    ...
    recalcRequisitionPricingTotal(instanceId);
  };

  window.recalcRequisitionPricingTotal = function(instanceId) {
    const tbody = document.getElementById('req-price-tbody-' + instanceId);
    if (!tbody) return;
    ...
    const totalEl = document.getElementById('req-price-total-' + instanceId);
    if (totalEl) {
